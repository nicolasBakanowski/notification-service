  ...
  .post( async ( req, res ) => {
    ...
    status.error && 
      res
        ...
        .send({ errorMessage: status.error })
    try {
      const log = await createLog(status)
      return res
        .status(httpStatus.CREATED)
        .send({ message: "Notification was succesfully logged.", log: log})
    } catch (err) {
      return res
        .status(httpStatus.INTERNAL_SERVER_ERROR)
        .send({message: "There was an error and the notification was not logged properly.", errorCode: err.code})
    }
  })
